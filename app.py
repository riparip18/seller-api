...
def home():
    ...
    return jsonify({"message": "Seller API aktif", "status": "ok"})
    ...
    try:
        ...
        if payload.get("message_type") == 2:
            ...
            if success:
                ...
                # print("[INFO] Transaksi berhasil diproses dan dikirim!")
                return jsonify({"status": "Transaksi berhasil diproses"}), 200
            else:
                # print("[ERROR] Gagal memproses transaksi.")
...
if __name__ == "__main__":
    try:
        ...
        # Tutup semua tunnel yang sudah ada sebelumnya
        try:
            ngrok.kill()
        except:
            pass
        
        # Kill proses ngrok secara manual jika masih ada
        try:
        ...
        # Buat tunnel baru dengan static domain
        public_url = ngrok.connect(5000, hostname="shining-stork-briefly.ngrok-free.app")
        print(" * ngrok tunnel:", public_url)
    except ImportError:
        print("[WARNING] pyngrok tidak terinstall. Jalankan: pip install pyngrok agar ngrok otomatis.")
    except Exception as e:
        print(f"[WARNING] Gagal menjalankan ngrok otomatis: {e}")
        print("[INFO] Solusi alternatif:")
        print("1. Hentikan semua ngrok process: pkill ngrok atau taskkill /F /IM ngrok.exe")
        print("2. Jalankan manual: ngrok http --url=shining-stork-briefly.ngrok-free.app 5000")
        print("3. Atau jalankan tanpa static domain: ngrok http 5000")
    
    try:
        app.run(port=5000, debug=True)
    except KeyboardInterrupt:
        print("\n[INFO] Aplikasi dihentikan")
        try:
            from pyngrok import ngrok
            ngrok.kill()
            print("[INFO] Tunnel ngrok ditutup")
        except:
            pass
